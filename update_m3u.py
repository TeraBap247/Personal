...
# --- Step 3: SonyLiv Section Update ---
def update_SonyLiv():
    def hash_list(lines):
        return hashlib.md5("\n".join(lines).encode()).hexdigest()

    SonyLiv_url = "https://raw.githubusercontent.com/drmlive/sliv-live-events/refs/heads/main/sonyliv.m3u"
    SonyLiv_data = requests.get(SonyLiv_url).text

    SonyLiv_lines = []
    lines = SonyLiv_data.strip().splitlines()
    i = 0
    while i < len(lines):
        line = lines[i]
        if '#EXTINF:' in line:
            # একাধিক EXTINF থাকলে ঠিক করা
            parts = line.split('#EXTINF:')
            if len(parts) > 2:
                line = '#EXTINF:' + parts[-1]  # শেষেরটা ধর
            if 'group-title="' in line:
                line = re.sub(r'group-title=".*?"', 'group-title="SonyLiv Live"', line)
            else:
                line = line.replace('#EXTINF:', '#EXTINF: group-title="SonyLiv Live",')
            SonyLiv_lines.append(line.strip())
            if i + 1 < len(lines) and lines[i + 1].startswith("http"):
                SonyLiv_lines.append(lines[i + 1].strip())
                i += 1
        i += 1
    ...
    while i < len(template_lines):
        line = template_lines[i]
        if line.startswith("#EXTINF:") and 'SonyLiv Live' in line:
            old_SonyLiv.append(line.strip())
            if i + 1 < len(template_lines) and template_lines[i + 1].startswith("http"):
                old_SonyLiv.append(template_lines[i + 1].strip())
                i += 1
        else:
            other_lines.append(line.rstrip())
        i += 1

    if hash_list(old_SonyLiv) != hash_list(SonyLiv_lines):
        with open("template.m3u", "w") as f:
            f.writelines([line + '\n' for line in other_lines])
            f.write('\n'.join(SonyLiv_lines) + '\n')
        print("SonyLiv আপডেট হয়েছে।")
    else:
        print("SonyLiv অপরিবর্তিত।")
...
safe_run("SonyLiv", update_SonyLiv)
