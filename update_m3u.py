import os import requests import re from datetime import datetime, timedelta import hashlib import json

--- ধাপ ১: বাইরের সোর্স থেকে .m3u ফাইল ডাউনলোড ---

source_url = "https://raw.githubusercontent.com/byte-capsule/Toffee-Channels-Link-Headers/main/toffee_OTT_Navigator.m3u" response = requests.get(source_url) source_data = response.text

--- ধাপ ২: নির্দিষ্ট চ্যানেলের block খুঁজে বের করা ---

channel_name = "Cartoon Network HD"

pattern = re.compile( rf'#EXTINF:-1.?,\s{re.escape(channel_name)}\s*\n(#EXTVLCOPT:.\n)?(#EXTHTTP:.\n)?(https?://.*)', re.MULTILINE ) match = pattern.search(source_data)

if match: new_vlcopt = match.group(1) or '' new_exthttp = match.group(2) or ''
...
else: print("Channel not found in source.")
