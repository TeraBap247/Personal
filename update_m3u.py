...
if match:
    ...
    new_fancode_lines = []
    fancode_lines = fancode_data.strip().splitlines()
    i = 0
    while i < len(fancode_lines):
        line = fancode_lines[i]
        if line.startswith("#EXTINF:"):
            ...
            if i + 1 < len(fancode_lines) and fancode_lines[i + 1].startswith("http"):
                new_fancode_lines.append(fancode_lines[i + 1].strip())
                i += 1
        i += 1
