...
def update_fancode():
    ...
    fancode_url = "https://fc.himelop-pro.top/fancode/playlist/playlist.m3u"
...
def update_sony_live_event():
    ...
    sony_live_event_url = "https://raw.githubusercontent.com/drmlive/sliv-live-events/refs/heads/main/sonyliv.m3u"
    sony_live_event_data = requests.get(sony_live_event_url).text
    sony_live_event_lines = []

    lines = sony_live_event_data.strip().splitlines()
    i = 0
    while i < len(lines):
        line = lines[i]
        if line.startswith("#EXTINF:"):
            if 'group-title="' in line:
                line = re.sub(r'group-title=".*?"', 'group-title="Sony Liv Live Event"', line)
            else:
                line = line.replace('#EXTINF:', '#EXTINF: group-title="Sony Liv Live Event",')
            sony_live_event_lines.append(line.strip())
            if i + 1 < len(lines) and lines[i + 1].startswith("http"):
                sony_live_event_lines.append(lines[i + 1].strip())
                i += 1
        i += 1
    ...
    while i < len(template_lines):
        line = template_lines[i]
        if line.startswith("#EXTINF:") and 'Sony Liv Live Event' in line:
            old_sony_live_event.append(line.strip())
            if i + 1 < len(template_lines) and template_lines[i + 1].startswith("http"):
                old_sony_live_event.append(template_lines[i + 1].strip())
                i += 1
        else:
            other_lines.append(line.rstrip())
        i += 1

    if hash_list(old_sony_live_event) != hash_list(sony_live_event_lines):
        with open("template.m3u", "w") as f:
            f.writelines([line + '\n' for line in other_lines])
            f.write('\n'.join(sony_live_event_lines) + '\n')
        print("sony_live_event আপডেট হয়েছে।")
    else:
...
safe_run("Sony Liv Live Event", update_sony_live_event)
